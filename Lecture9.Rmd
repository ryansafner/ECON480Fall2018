...
\begin{columns}
  \begin{column}[c]{9cm}
    \begin{itemize}
      ...
      \end{itemize}
      \item<3-> Data work is a science, but it should also be \textbf{art}!
      \item<4-> I will \emph{not}  grade you on the beauty of your graphics (but we're all secretly judging!)
      \begin{itemize}
        \item<5-> \texttt{hist()} and \texttt{plot()} are fine, you are not required to use \texttt{ggplot2} (but you really should! )
      \end{itemize}
    \end{itemize}
  \end{column}
  \begin{column}[c]{4cm}
